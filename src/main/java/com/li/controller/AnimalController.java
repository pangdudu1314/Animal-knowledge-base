...
public class AnimalController {
  ...
  @Autowired
  public IAnimalCheckService animalCheckService;
...
*/
  ...
  public String selectAdmin(String name, HttpServletRequest request, HttpServletResponse response)
...
//访问量
  ...
  public String bianji(String name, HttpServletRequest request, HttpServletResponse response)
      throws IOException {
  ...
  public void updateAnimalMethod(String name, String file, String intro,
    ...
    try {
      ...
      boolean existUncheckAnimal = animalCheckService.existUnCheckAnimal(name);
      if (existUncheckAnimal) {
        map.put("result", "此动物名称有未审核动物，不允许再次编辑");
      } else {
        AnimalInfo animalInfo=  iAnimalService.selectName(name);
  ...
  public void deleteAnimalMethod(String name, HttpServletRequest request,
      HttpServletResponse response) {
    try {
      Map map = new HashMap();
      ...
      boolean existUncheckAnimal = animalCheckService.existUnCheckAnimal(name);
      if (existUncheckAnimal) {
        map.put("result", "此动物名称有未审核动物，不允许再次删除");
      } else {
        AnimalInfo animalInfo=  iAnimalService.selectName(name);
        AnimalCheck animalCheck = new AnimalCheck();
        animalCheck.setId(IDRandomUtils.createRandomStr());
        animalCheck.setAnimalName(name);
        animalCheck.setDataFrom("删除信息");
        animalCheck.setStatus("未处理");
        animalCheck.setAnimalImage(animalInfo.getImage());
  ...
  public void addAnimalMethod(String kemu, String name, String file, String message,
    ...
    try {
      ...
      if (existAnimalInfo.getName() != null) {
       ...
       /* AnimalInfo animalInfo = new AnimalInfo();
        ...
        boolean existUncheckAnimal = animalCheckService.existUnCheckAnimal(name);
        if (existUncheckAnimal) {
          map.put("result", "此动物名称有未审核动物，不允许再次添加");
        } else {
          AnimalCheck animalCheck = new AnimalCheck();
          animalCheck.setId(IDRandomUtils.createRandomStr());
          animalCheck.setAnimalName(name);
          animalCheck.setDataFrom("添加信息");
          animalCheck.setStatus("未处理");
          animalCheck.setAnimalImage("images//" + file);
          animalCheck.setKemu(kemu);
          animalCheck.setAnimalIntro(message);
          animalCheckService.addAnimalCheck(animalCheck);
          map.put("result", "添加到待审核");
        }
      }
